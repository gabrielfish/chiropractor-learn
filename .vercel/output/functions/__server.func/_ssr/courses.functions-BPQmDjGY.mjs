...
const publishAllDraftCourses_createServerFn_handler = createServerRpc({
  id: "ff779bbab4ff04bf67d5513ba10a0b57fb531258368564f391e9b1a80eb8978c",
  name: "publishAllDraftCourses",
  filename: "src/lib/courses.functions.ts"
}, (opts) => publishAllDraftCourses.__executeServer(opts));
const publishAllDraftCourses = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).handler(publishAllDraftCourses_createServerFn_handler, async ({
  context
}) => {
  const {
    supabase,
  ...
  const {
    supabaseAdmin: _supabaseAdmin4
  } = await import("./client.server-IqT0ZZNy.mjs");
  const supabaseAdmin = _supabaseAdmin4;
  const {
    data: updated,
    error
  } = await supabaseAdmin.from("courses").update({
    status: "published"
  }).eq("status", "draft").select("id");
  if (error) throw new Error(error.message);
  return {
...
export {
  ...
  publishAllDraftCourses_createServerFn_handler,
